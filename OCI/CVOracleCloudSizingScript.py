...
# OCI profile names should only contain alphanumeric characters, underscores, and hyphens
# to prevent command injection attacks. This follows OCI CLI naming conventions.
...
def _validate_profile_name(value):
    if not isinstance(value, str) or not _PROFILE_NAME_RE.match(value):
...
if __name__ == "__main__":
    ...
    for arg in args:
        if arg.startswith("--profile="):
            raw_profile_name = arg.split("=")[1]
            try:
                profile_name = _validate_profile_name(raw_profile_name)
